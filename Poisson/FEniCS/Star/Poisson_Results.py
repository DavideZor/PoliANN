...
for i in degree_list:
  ...
  for j in res_list:
    ...
    u_ex = Expression('exp(-(x[0]*x[0] + x[1]*x[1]))', degree = i + 1)
    f = Expression('4*(1-(x[0]*x[0] + x[1]*x[1]))*exp(-(x[0]*x[0] + x[1]*x[1]))', degree = i + 1)

    # Re-shaped star
    u_ex = Expression('sin(pi*x[0]) * sin(pi * x[1])', pi = np.pi, degree = i + 1)
    f = Expression('2 * pi * pi * sin(pi*x[0]) * sin(pi * x[1])', pi = np.pi, degree = i + 1)
  ...
  s = s + '\\label{{tab:PoissonStar{}}}'.format(int(i)) + '\n'
  s = s + '\\end{table}' +  '\n'
  s = s + '\n'
...
plt.savefig('PoissonStarDegreevsTime.pdf')
...
plt.legend(framealpha=1, frameon=True);
plt.savefig('PoissonStarDegreeErrorL2.pdf')
...
plt.plot(h_vec, h_vec**(-4),'-.', label='$h^{-4}$')
plt.legend(framealpha=1, frameon=True);
plt.savefig('PoissonStarDegreeErrorH1.pdf')
...
plt.title(tit)
plt.savefig('PoissonStarDegree4Solution.pdf')

times = np.array(times)
times = times.reshape((len(res_list), -1))

with open('FEniCSPoissonStarErrorsLateX.txt', 'w') as f:
  f.write(s)
...
plt.title(tit)
plt.savefig('PoissonStarSolutionError.pdf')

tit_points = "FEniCSPoissonStar{}".format(Num_Points)
